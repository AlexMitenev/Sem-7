    ...
    internal partial class LibraryView : Form
    {
        ...
        private void RemoveFilmsClick(object sender, EventArgs e)
        {
            ...
            foreach (DataGridViewRow row in FilmsGrid.Rows.Cast<DataGridViewRow>().Where(row => row.Selected))
            {
                ...
                var mess = string.Format("Do you want to remove '{0}'", filmName);
                if (MessageBox.Show(mess, Resources.RemoveFilms, MessageBoxButtons.YesNo) == DialogResult.No)
                {
                    isRemoved = false;
                    break;
                }
            ...
            {
                foreach (DataGridViewRow row in rowForRemove)
                {
